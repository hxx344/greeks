...
function quoteLegComparison(quote, side, state) { const requested = new Map((state.legs || []).map((leg) => [leg.symbol, leg])); const chain = new Map((window.__latestChain || []).map((item) => [item.symbol, item])); return (quote[side === 'Buy' ? 'quoteBuyList' : 'quoteSellList'] || []).map((item) => { const leg = requested.get(item.symbol) || {}; const takerSide = side === 'Sell' ? (leg.side || 'Buy') : ((leg.side || 'Buy') === 'Buy' ? 'Sell' : 'Buy'); const market = chain.get(item.symbol) || {}; const reference = Number(takerSide === 'Sell' ? market.bid : market.ask) || Number(market.mark_price) || 0; const quoted = Number(item.price || 0); const diff = quoted - reference; return `<div class="rfq-leg-row"><strong>${esc(item.symbol)}</strong><span>${takerSide} ${quoted.toFixed(4)}</span><span>链 ${reference.toFixed(4)}</span><b class="${diff >= 0 ? 'rfq-diff-positive' : 'rfq-diff-negative'}">差 ${diff >= 0 ? '+' : ''}${diff.toFixed(4)}</b></div>`; }).join(''); }
function chainNet(state) { const requested = new Map((state.legs || []).map((leg) => [leg.symbol, leg])); const chain = new Map((window.__latestChain || []).map((item) => [item.symbol, item])); return (state.legs || []).reduce((sum, leg) => { const market = chain.get(leg.symbol) || {}; const price = Number(leg.side === 'Sell' ? market.bid : market.ask) || Number(market.mark_price) || 0; return sum + (leg.side === 'Sell' ? 1 : -1) * price * Number(leg.qty || 0); }, 0); }
function quoteNetDiff(quote, state) { return quoteNet(quote, 'Sell', state) - chainNet(state); }
