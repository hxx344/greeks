...
function quoteLegComparison(quote, side, state) { const requested = new Map((state.legs || []).map((leg) => [leg.symbol, leg])); const chain = new Map((window.__latestChain || []).map((item) => [item.symbol, item])); return (quote[side === 'Buy' ? 'quoteBuyList' : 'quoteSellList'] || []).map((item) => { const leg = requested.get(item.symbol) || {}; const takerSide = side === 'Sell' ? (leg.side || 'Buy') : ((leg.side || 'Buy') === 'Buy' ? 'Sell' : 'Buy'); const market = chain.get(item.symbol) || {}; const reference = Number(takerSide === 'Sell' ? market.ask : market.bid) || Number(market.mark_price) || 0; const quoted = Number(item.price || 0); const diff = quoted - reference; return `<div class="rfq-leg-row"><strong>${esc(item.symbol)}</strong><span>${takerSide} ${quoted.toFixed(4)}</span><span>链 ${reference.toFixed(4)}</span><b class="${diff >= 0 ? 'rfq-diff-positive' : 'rfq-diff-negative'}">差 ${diff >= 0 ? '+' : ''}${diff.toFixed(4)}</b></div>`; }).join(''); }
function quoteSpread(quote, state) { const requested = new Map((state.legs || []).map((leg) => [leg.symbol, leg])); const chain = new Map((window.__latestChain || []).map((item) => [item.symbol, item])); return (quote.quoteSellList || []).reduce((sum, item) => { const leg = requested.get(item.symbol) || {}; const market = chain.get(item.symbol) || {}; const reference = Number((leg.side || 'Buy') === 'Sell' ? market.ask : market.bid) || Number(market.mark_price) || 0; return sum + Math.abs(Number(item.price || 0) - reference); }, 0); }
function renderRfq(state) {
  if (!state || !state.rfq_id || ['Canceled', 'Expired', 'Filled', 'Failed'].includes(state.status)) { $('rfqStatus').textContent = state?.status || '未创建'; $('rfqId').textContent = '--'; $('rfqExpires').textContent = '--'; $('rfqQuoteCount').textContent = '0'; $('rfqQuotes').className = 'rfq-quotes empty'; $('rfqQuotes').textContent = '暂无活动 RFQ'; return; }
  $('rfqStatus').textContent = state.status || '--'; $('rfqId').textContent = state.rfq_id; $('rfqExpires').textContent = state.expires_at ? new Date(Number(state.expires_at)).toLocaleTimeString('zh-CN') : '--'; const quotes = (state.quotes || []).slice().sort((a, b) => quoteSpread(a, state) - quoteSpread(b, state)); $('rfqQuoteCount').textContent = String(quotes.length);
  $('rfqQuotes').className = quotes.length ? 'rfq-quotes' : 'rfq-quotes empty'; $('rfqQuotes').innerHTML = quotes.length ? quotes.map((quote) => { const legCount = quoteLegCount(quote); const complete = legCount >= (state.legs || []).length; const disabled = complete ? '' : ' disabled title="报价未覆盖全部策略腿"'; return `<div class="rfq-quote"><div><strong>${esc(quote.deskCode || '做市商')} · ${legCount}/${(state.legs || []).length} 腿</strong><span>${esc(quote.status || '--')} · 到期 ${quote.expiresAt ? new Date(Number(quote.expiresAt)).toLocaleTimeString('zh-CN') : '--'}</span></div><div class="rfq-quote-values"><span>Buy 净额 ${quoteNet(quote, 'Buy', state).toFixed(4)}</span><span>Sell 净额 ${quoteNet(quote, 'Sell', state).toFixed(4)}</span><button class="button ghost rfq-execute" data-rfq="${esc(quote.rfqId || state.rfq_id)}" data-quote="${esc(quote.quoteId || '')}" data-side="Buy"${disabled}>执行 Buy</button><button class="button ghost rfq-execute" data-rfq="${esc(quote.rfqId || state.rfq_id)}" data-quote="${esc(quote.quoteId || '')}" data-side="Sell"${disabled}>执行 Sell</button></div><div class="rfq-compare-title">Buy 报价方向 · 你的四腿成交方向</div><div class="rfq-leg-compare">${quoteLegComparison(quote, 'Buy', state) || '<span>无 Buy 方向报价</span>'}</div><div class="rfq-compare-title">Sell 报价方向 · 你的四腿成交方向</div><div class="rfq-leg-compare">${quoteLegComparison(quote, 'Sell', state) || '<span>无 Sell 方向报价</span>'}</div></div>`; }).join('') : '等待做市商报价';
