...
async function load() {
  try {
    ...
    $('btcPrice').textContent = preview.btc_price ? money(preview.btc_price) : '--';
...
function quoteTotal(quote, side) { return (quote[side === 'Buy' ? 'quoteBuyList' : 'quoteSellList'] || []).reduce((sum, item) => sum + Number(item.price || 0) * Number(item.qty || 0), 0); }
function renderRfq(state) {
  ...
  $('rfqQuotes').className = quotes.length ? 'rfq-quotes' : 'rfq-quotes empty'; $('rfqQuotes').innerHTML = quotes.length ? quotes.map((quote) => `<div class="rfq-quote"><div><strong>${esc(quote.deskCode || '做市商')}</strong><span>${esc(quote.status || '--')} · 到期 ${quote.expiresAt ? new Date(Number(quote.expiresAt)).toLocaleTimeString('zh-CN') : '--'}</span></div><div class="rfq-quote-values"><span>Buy ${quoteTotal(quote, 'Buy').toFixed(4)}</span><span>Sell ${quoteTotal(quote, 'Sell').toFixed(4)}</span><button class="button ghost rfq-execute" data-rfq="${esc(quote.rfqId || state.rfq_id)}" data-quote="${esc(quote.quoteId || '')}" data-side="Buy">执行 Buy</button><button class="button ghost rfq-execute" data-rfq="${esc(quote.rfqId || state.rfq_id)}" data-quote="${esc(quote.quoteId || '')}" data-side="Sell">执行 Sell</button></div></div>`).join('') : '等待做市商报价';
...
async function createRfq() { const counterparties = $('rfqCounterparties').value.split(',').map((item) => item.trim()).filter(Boolean); try { await getJson('/api/rfq/create', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({confirm_live:$('confirm').checked, counterparties, quantity:Number($('quantity').value)})}); await loadRfq(); } catch (error) { window.alert(error.message); } }
