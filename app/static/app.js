...
function quoteLegCount(quote) { return new Set([...(quote.quoteBuyList || []), ...(quote.quoteSellList || [])].map((item) => item.symbol)).size; }
...
function renderRfq(state) {
  ...
  $('rfqStatus').textContent = state.status || '--'; $('rfqId').textContent = state.rfq_id; $('rfqExpires').textContent = state.expires_at ? new Date(Number(state.expires_at)).toLocaleTimeString('zh-CN') : '--'; const quotes = state.quotes || []; $('rfqQuoteCount').textContent = String(quotes.length);
