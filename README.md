...
“策略实盘收益”按每次组合开仓列出成交、权利金、开平仓手续费、交割手续费及收益。展开组合可查看逐笔成交。主曲线累计已经完全平仓或交割且记录核实完整的组合净收益；持仓中的已实现部分、剩余浮盈亏和待核对组合单独展示，不提前计入曲线。胜率和回撤均基于这条已结束组合曲线，不代表账户总资产收益率或盘中最大回撤。

成交台账持久化在现有状态文件内，按成交 ID 去重，不受最近 100 条成交列表限制，重启后继续累计。后台每 30 秒同步一次；初次从已知策略最早开仓前一天开始分周补齐（最多回查 729 天，无已知策略时从最近 7 天开始），页面显示起点和同步进度。交易所交割记录仅在数量、方向和归属核实后使用，直接采用其已含费用的 `totalPnl`，不会再重复扣费。无法归属的手动平仓、缺失的历史记录、未完成的开平仓会保留待核对状态；历史不完整时不声称覆盖账户全部业绩。

实盘和测试网按状态文件网络隔离；模拟交易不会计入。USDT、USDC 分别统计，不混合账户充值、提现和无关交易。收益台账同步故障保留本地记录并提示重试。部署更新后需重启后端，才能启用持久化台账和新接口。
